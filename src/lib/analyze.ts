import type { Signal, Source } from "../types.ts";
...
export function analyzeText(title: string, description = "") {
  ...
  return { topic, sentiment } as const;
}

export function makeSignal(input: Omit<Signal, "topic" | "sentiment">): Signal {
  return { ...input, ...analyzeText(input.title, input.description) };
}

export function summarize(signals: Signal[]): string[] {
  const count = (key: keyof Signal) => Object.entries(signals.reduce<Record<string, number>>((a, s) => {
    const value = String(s[key] ?? "Unknown"); a[value] = (a[value] ?? 0) + 1; return a;
  }, {})).sort((a, b) => b[1] - a[1]);
...
export const sourceLabel = (source: Source) => ({ "Hacker News": "HN", "GitHub Trending": "GH", Reddit: "RD", "npm Registry": "NPM", "DEV Community": "DEV", Lobsters: "LOB", X: "X", arXiv: "ARX", "Stack Overflow": "SO", InfoQ: "IQ", YouTube: "YT" })[source];
