...
export default function App() {
  ...
  const runLocalAi = async () => {
    ...
    try {
      ...
      const classifier = await pipeline("sentiment-analysis", "Xenova/distilbert-base-uncased-finetuned-sst-2-english", { dtype: "q8" });
  ...
  const ghCount = data.signals.filter(s=>s.source === "GitHub Trending").length;
  return <div className="shell">
    ...
    <main>
      <section className="hero"><div className="hero-copy"><div className="kicker">Independent technology intelligence</div><h1>A quieter way to read<br/>the developer internet.</h1><p>DevSignal collects two noisy public feeds and turns them into one considered daily brief—what is gaining attention, which tools are moving, and what is mostly hype.</p><a className="jump" href="#dashboard">Read today’s signal <span>↓</span></a></div><aside className="method"><span>Today’s sample</span><dl><div><dt>{data.signals.length}</dt><dd>signals reviewed</dd></div><div><dt>{hnCount}</dt><dd>Hacker News stories</dd></div><div><dt>{ghCount}</dt><dd>trending repositories</dd></div></dl><p>Collected automatically. Classified with {data.mode === "openai" ? "OpenAI" : "transparent keyword rules"}. Every item links to its source.</p></aside></section>
      <section className="insights"><div className="section-label"><span>{localStatus === "ready" ? "Local AI active" : data.mode === "openai" ? "AI briefing" : "Automated briefing"}</span><small>{localStatus === "ready" ? "Sentiment analyzed privately in this browser" : data.mode === "openai" ? `Analyzed by ${data.model ?? "OpenAI"} · source links retained` : "Keyword fallback · free local AI is available below"}</small></div><div className="insight-grid">{data.summary.map((x,i)=><article key={x}><b>{i+1}</b><p>{x}</p></article>)}</div></section>
      <section id="dashboard" className="dashboard"><div className="section-head"><div><div className="section-label"><span>01 / Overview</span></div><h2>What the feed is saying</h2></div><div className="filters"><div className="search"><Search size={15}/><input aria-label="Search signals" placeholder="Search the index" value={query} onChange={e=>setQuery(e.target.value)}/></div>{["All","Hacker News","GitHub Trending"].map(x=><button className={source===x?"active":""} onClick={()=>setSource(x)} key={x}>{x}</button>)}</div></div>
