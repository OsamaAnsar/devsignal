...
export default function App() {
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => { if (autoAiStarted.current) return; autoAiStarted.current = true; void runLocalAi(); }, 900);
  ...
  return <div className="shell">
    ...
    <main>
      ...
      <section className="insights"><div className="section-label"><span>{localStatus === "ready" ? "Live AI briefing" : localStatus === "loading" ? "AI trend scan starting" : data.mode === "openai" ? "AI briefing" : "Daily briefing"}</span><div className="brief-actions"><small>{localStatus === "ready" ? "Analyzed privately in this browser" : localStatus === "loading" ? `Downloading private AI model · ${localProgress}%` : data.mode === "openai" ? `Analyzed by ${data.model ?? "OpenAI"} · source links retained` : "AI analysis starts automatically after the page loads"}</small><button onClick={shareBrief}>{shared ? <Check size={13}/> : <Share2 size={13}/>} {shared ? "Ready to share" : "Share brief"}</button></div></div><div className="insight-grid">{activeSummary.map((x,i)=><article key={x}><b>{i+1}</b><p>{x}</p></article>)}</div></section>
